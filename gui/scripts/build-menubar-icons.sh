...
function generateICO() {
  ...
  for SIZE in 16 24 32 48; do
    ...
    rsvg-convert -o "$_TMP.png" -w $SIZE_WO_PADDING -h $SIZE_WO_PADDING "$_IN"
  ...
  convert "$TMP/16.png" "$TMP/24.png" "$TMP/32.png" "$TMP/48.png" "$TMP/16-8.png" "$TMP/24-8.png" \
    "$TMP/32-8.png" "$TMP/48-8.png" "$TMP/16-4.png" "$TMP/24-4.png" "$TMP/32-4.png" \
    "$TMP/48-4.png" $COMPRESSION_OPTIONS "$_OUT"
  rm "$TMP/16.png" "$TMP/24.png" "$TMP/32.png" "$TMP/48.png" "$TMP/16-8.png" "$TMP/24-8.png" \
    "$TMP/32-8.png" "$TMP/48-8.png" "$TMP/16-4.png" "$TMP/24-4.png" "$TMP/32-4.png" \
    "$TMP/48-4.png"
}
...
function generate() {
  ...
  # # Linux colored
  generatePNG "$IN" "$LINUX/$OUT.png" 32 4

  # # Linux white
  generatePNG "$WHITE_SVG" "$LINUX/${OUT}_white.png" 32 4
